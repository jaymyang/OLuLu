...
# 對新連入的客戶端。發送指令 '9' 要求回報身分編號
    ...
    while True:
        ...
        try:
            ...
            #print(message_part1,message_buffer)#用來追蹤
            #接著將字串轉成陣列
    ...
    if bed_n: #輸入床號完成，接下來要更動pt_info_data的東西
        for i in pt_info_data:
            if pt_info_data[i]['Bed'] == bed_n: #抓到所要的床號
                for j in pt_info_data:
                    if pt_info_data[j]['client_name'] == message_list[-1]:
                        pt_info_data[j]['client_name'] == pt_info_data[i]['client_name']
                    else:
                        pass
                pt_info_data[i]['client_name']= message_list[-1] #將字典中該床號的client_name設為新連線裝置的號碼
                pt_info_data[i]['client_IP']=str(client_address[0]) #寫入pt_info_data中
                predefined_client= True
                update_button_text(i)
                bed_n='' #重設為空
