                ...
                if pt_info_data[k]['pt_number'] !='請輸入病歷號': #有登錄的
                    for j in range(len(data)): #找尋資料相符的
                        if data[j]['pt_number']== pt_info_data[k]['pt_number']: #找到相符的資料
                            ...
                            saving_pt_info_data(pt_info_data)
                            decreasing_o_i_d=''
                ...
                for i in range(len(data)):    #記憶體內的資料，button_on_display為主
                    if data[i]['pt_number'] == pt_info_data[button_on_display]["pt_number"] :
                        if len(data[i]["weight"]) < 10:
                            ...
                            cal_weight_ver_2=calculate_weight(data[i]["weight"][-10:])    #用最近10分鐘資料這個串列來計算
                            cal_weight_ver_1=calculate_weight_changes(data[i]['weight'][-10:]) #舊方法計算重量
                print('新方法試算10分鐘重量變化',cal_weight_ver_2,'舊方法試算10分鐘重量變化',cal_weight_ver_1)
            Analysis_text.config(text=f"漸減：{decreasing_list}，重/10分：新{cal_weight_ver_2}、舊{cal_weight_ver_1}") ##顯示在下排。新／舊
...
# ======================存名單函數。======================
def saving_pt_info_data(pt_data):
    try:
        with open('pt_info_data.csv', 'w', newline='', encoding='utf-8') as csvfile:
            # 取得 CSV 檔案的欄位名稱 (從字典的第一個值中取得 keys)
            fieldnames = list(pt_data[0].keys())
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            # 寫入 CSV 檔案的標題列
            writer.writeheader()
            # 寫入資料列
            for data in pt_info_data.values():
                writer.writerow(data)
        print(f"資料已成功儲存至 pt_info_data.csv")
    except Exception as e:
        print(f"儲存pt_info_data.csv檔案時發生錯誤: {e}")
    
# ======================資料分析。======================
# Function to calculate weight changes#需要偵測重量突減以及異常大量。
# 0.3版搬來的，因為還不確定要用哪一個，先併行比較一下
def calculate_weight_changes(weight_FLUID):
    weight_sum=0
    ...
    if len(weight_FLUID)>9:
        ...
        weight_recent=weight_FLUID[-10:] #工作用串列
        small_volume=np.max(weight_recent)-np.min(weight_recent)
...
def calculate_weight(cal_weight):#以下是計算重量的新方法
    turn_point_1=0
    difference=0
    diff_weight=0
    new_diff=0 
        ...
        if response:
            ...
            if data_to_be_saved!=[]:
                try:
                    ...
                    saving_pt_info_data(pt_info_data) #現在的病人資料表
                    print('已登出並存檔',data)
                except Exception as e:
...
def one_eight_switch(switch_1_8): #第一步：準備要畫圖的資料點
    ...
    if data_to_be_displayed!=[]:
        if data_to_be_displayed["weight"] !=[] and button_on_display is not None: #有重量，且有選擇到某位病人
            ...
            if len(data_to_be_displayed['time'])<60: #如果不滿60個點，就去呼叫檔案補足
...
def getfiledata(y,formatted_time_list): #讀檔函式
    global pt_info_data,button_on_display
    try:
        ...
        for i, time_point in enumerate(formatted_time_list):            
            if y[i] == 0 and time_point in csv_data:# 只補上 y[i] == 0的點（表示記憶體內無資料）
...
#顯示分析資料，這在下排。本來因為單純顯示，所以只用label，但發現跟上面的大小不搭，試看看sticky=tk.W+tk.E。並且跟上面的一樣做成平面的按鈕，以免使用者混淆。
