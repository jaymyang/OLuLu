...
#                        #-2-2 接收感測器訊息與訊息格式化 #                         #
#-+--+--+--+--+-+--+--+--+--+-+--+--+--+--+-+--+--+--+--+-+--+--+--+--+-+--+--+--+--#
#下面是處理socket中的東西的範例－－雖然我不懂為什麼contains前後要加底線。
...
def message_R(message_list, client_address): 
    global client_dict,client_list, connected_clients  
...
def scan_clients():
    ...
    while True:
        ...
        if current_time.tm_sec == 1: # 每分鐘的01秒執行掃描
            ...
            #理論上是要有病歷號才能接收，所以對於沒有病歷號的IP要怎麼處理？就不發送？還是繼續發送但丟棄？
            #如有問題，應考慮在message_R那邊加上「切斷不在名單中的客戶端」功能
...
def display_info(button_number, displayed):
    ...
    try:# 重置先前選取的按鈕顏色
        ...
        #        
        #        if data_to_be_displayed["weight"] !=[]:
        #            y_data_points=one_eight_switch(one_eight_selection,data_to_be_displayed)[0] #切換1/8小時與準備繪圖資料（預設toggle值為1）[0]是配合回傳資料的
