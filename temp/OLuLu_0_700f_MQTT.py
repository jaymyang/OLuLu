...
mqtt_port = 1883
topic_sub="olulu/response"
topic_pub="olulu/command"
...
#現在留下處理選單的功能    
def message_R(device_id): #ID理論上已經處理好了，直接用
    ...
    if device_id in client_list:
        for j in pt_info_data:
            if device_id == pt_info_data[j]['client_name']:
                new_device=False
                break
            else:
                new_device=True
        if new_device==True:
            for k in client_dict:
                if client_dict[k][0]== device_id[-2:]:
                    ...
                    new_connected_clients.add(device_id[-2:]) #僅顯示編號
                else:
                    pass
...
def scan_clients(client):
    ...
    while True:        
        ...
        if closing==True:
            break
        if current_time.tm_sec %30 == 0: # 每分鐘的00,30秒執行掃描，送心跳包確認連線狀況
            print("scan_clients: 開始檢查連線狀態")
            if len(new_connected_clients) == 0:  # 只在沒有感測器回應時發送9
                client.publish(topic_pub, "9")
            time.sleep(1.5)
        if current_time.tm_sec == 5: # 每分鐘的05秒執行掃描
            print("scan_clients: 開始發送命令")
        ...
        if current_time.tm_min in min_for_saving and current_time.tm_sec == 35 and not saved and data !=[]:
            for j in range(len(data)): #檢查既有資料名單。由於data字典中是以IP為key,省略了病歷號，雖然在重新連線時可以自動把資料扔進data，但在存檔時就必須去pt_info_data取得病歷號用來存檔
                for k in range(len(pt_info_data)): #所有的感測器；由於clien_lis 也是跟這個同步，所以這回圈不一定要用_info_daa
                    if pt_info_data[k]['pt_number'] !='請輸入病歷號': 
...
try:
    ...
    for i in range(len(x)):
        ...
        print(f"x = {x[i]}, y_pred = {y_pred:.2f}, y_actual = {y[i]}")
