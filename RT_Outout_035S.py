#由於大部病人實際上每分鐘重量變化甚微，每分鐘計算似無意義。本版目標是要簡化資料數量
...
def saving_data(saving_time, saving_weight, cutting_index):
    if saving_weight:
        ...
        with open(file_name, 'a', newline='') as csvfile:
            ...
            print("過去30分鐘數據存檔完成,file_weight",file_weight)
    ...
    while True:
        try:  #首先判定時間，以確保每分鐘只會執行一次以下程式，避免資料過多或重複
            if time.localtime()[4] != current_minute: #current_time代表以下程式區塊所執行的時間。time.localtime[4]不等於current_time時，表示是新的分鐘
                ...
                weight_flag=0
                one_min_weight=[]
                if time.localtime()[5] == 00:#時間之秒為0，收集資料
                    one_min_weight=get_weight(weight_flag)
                    weight_flag=one_min_weight[1]
                    making_sound(392,50,0.1,1)
                
                    if one_min_weight[0] == -999 or None: #要確定不是空串列
                        weight_FLUID.append(weight_FLUID[-1]) #本分鐘回傳為空，就重複上一分鐘的數字。但為了這個，所以在main一開始要先做出一組數字，免得-999到底                        
                    else:
                        weight_FLUID.append(one_min_weight[0])  #加入重量紀錄主串列weight_Fluid
...
if __name__ == '__main__':
    ...
    for m in range(0,4,1):
        making_sound(260+m*30,900,0.1,1)
    for m in range(0,4,1):
        making_sound(380-m*30,900,0.1,1)
