            ...
            try:
                ...
                if kind == "mqtt_device":
                    ...
                    if mtype == "status":
                        ...
                        # 【監視器 1】看看 Broker 到底有沒有認出這個感測器是哪張床的？
                        print(f"[寫檔追蹤] 收到感測器 {sensor_id} 重量: {val} | 對應床位: {target_bed.name if target_bed else '找不到綁定床位'}")
                        
                        if target_bed:
                            try:
                                # 嘗試轉換數字並存檔
                                weight_float = float(val)
                                target_bed.add_data(datetime.datetime.now().strftime("%Y-%m-%d %H:%M"), weight_float)
                                target_bed.is_online = True
                                print(f"[寫檔追蹤] 成功呼叫存檔，病歷號: {target_bed.patient_id}")
                            except Exception as e: 
                                # 【監視器 2】如果有錯，立刻大聲報警，絕對不准 pass！
                                print(f"[致命錯誤] 寫入 CSV 時發生崩潰: {e}")
