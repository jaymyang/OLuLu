...
#        print(f"Windows Mosquitto broker 已在後台啟動，PID: {process.pid}")
...
#else:
    # 代表 Linux 或 Mac 系統
#    print("目前為非 Windows 系統，類Unix系統可假設 Mosquitto 已由系統自動啟動。")
...
# Viewer專用：指向主機的共用資料夾路徑以讀取主機的設定檔跟歷史資料)；注意：路徑前面的 r 不能省略，這代表「原始字串」，避免斜線被當成程式碼
SHARED_FOLDER = r"Z:" 

CONFIG_FILE = f"{SHARED_FOLDER}\\bed_config.json"
THEMES_FILE = f"{SHARED_FOLDER}\\themes.csv"
...
#======================(viewer) ===================
    ...
    def load_history_from_csv(self):
        ...
        if not self.patient_id: return
#======================(viewer) ===================        
        # Viewer 指向網路共享磁碟機 (在viewer的機器端指定的掛載代號，本例為Z）
        base_dir = r"Z:/" 
...
class OLuLuSystem(tk.Tk):
    ...
    def toggle_time_view(self):
        ...
        self.update_chart_display()
#=============================(以下viewer專用）===============================
    def on_bed_click(self, bed_name): #viewer直接畫圖
        self.current_viewing_bed = bed_name
        self.btn_toggle_time.pack(side=tk.LEFT, padx=10) # 保留切換時間的按鈕
        self.update_chart_display()
...
#=============================(以上viewer專用）===============================
    ...
    def update_chart_display(self):
        ...
        self.canvas.create_line(padding, zero_y, w-padding, zero_y, width=2, fill=t["text_dark"])  # 畫 X 軸 (精確定位於 0)
