...
class BedModel:
    ...
    def __init__(self, name):
        ...
        # 儲存歷史數據，格式為 list of tuple: [("YYYY-MM-DD HH:MM", 500), ...]
        self.history = []    
    ...
    def load_history_from_csv(self):
        ...
        # 1. 取得目前 python 程式所在的資料夾路徑，確保萬無一失
        ...
        # 2. 準備三種常見的編碼來開鎖 (解決 Excel 或舊系統存檔的問題)
...
class OLuLuSystem(tk.Tk):
    ...
    def update_chart_display(self):
        ...
        # ================= 1. 基本變數與邊距設定 =================
        padding = 40  # 畫布邊緣留白距離 (重要：必須先定義，後續運算才不會報錯)
        bottom_y = h - padding
        ...
        if current_max_weight > 500:
            ...
            current_line_color = t.get("chart_line_high", "#7BED9F") # 嫩綠色 (低明度，表示量大)
    ...
    def timer_sync_clock(self):
        """定時器：每秒檢查，於每分鐘 05 秒時發送同步廣播"""
        now = datetime.datetime.now()
        if now.second == 5: self.client.publish("olulu/all/trigger", "1")
