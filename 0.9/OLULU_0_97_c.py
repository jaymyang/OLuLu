...
# ================== 【Controller 拔除了 Mosquitto 啟動邏輯】 ==================
# Controller 假設 Broker 已經在某處  啟動了。
...
THEMES_FILE = "themes.csv"     # 佈景主題還是 Controller 自己管
...
# ================= 資料格式設定 (Controller 版) =================
class BedModel:
    """代表單一床位，Controller 版只負責在記憶體中暫存 Broker 傳來的資料"""
    def __init__(self, name):
        self.name = name           
        self.sensor_id = None      
        self.patient_id = None     
        self.history = []    # 由 Broker 傳來的歷史資料填入
        self.is_online = False     
        self.is_alert = False      

    def add_data(self, timestamp, weight):
        """處理即時收到的新資料，不再做 CSV 存檔"""
...
class OLuLuSystem(tk.Tk):
    def __init__(self):
        ...
        # 為這台 Controller 產生專屬 ID (例如: Station_xxxx)
        ...
        if os.name == 'nt':
            ...
            window_height = self.winfo_screenheight() - 40 
            start_x = 0
            start_y = 0
        ...
        self.title_label = tk.Label(self.title_bar, text=" OLuLu 遠端監控儀表板 (Controller)", bg="#2F3542", fg="white", font=("Arial", 12, "bold"))
        ...
        self.apply_rounded_corners(window_width, window_height, 20) 
        ...
        # ================== MQTT 客戶端設定 (Controller) ==================
        # 不指定 client_id，讓 Paho 隨機生成，避免多個 Controller 互相踢下線
        ...
        try:
            ...
            print("Controller MQTT 連線機制已啟動...")
        except Exception as e:
            print(f"MQTT 啟動連線發生錯誤: {e}")

        # 【移除了定時向 Arduino 發指令的執行緒，交由 Broker 負責】
    ...
    def load_images(self): 
        ...
        self.img_startup, self.startup_w = get_resized_image(path_startup, int(screen_h * 0.50))
        path_standby = os.path.join(BASE_DIR, "copyright_2.jpg")
        self.img_standby, self.standby_w = get_resized_image(path_standby, int(screen_h * 0.50))
    ...
    def on_bed_click(self, bed_name):
        ...
        if bed.sensor_id is None:
            ...
            # 向 Broker 發出「索取歷史資料」的請求，畫圖的工作交給 on_message 處理
    ...
    def show_login_dialog(self, bed_name):
        ...
        def confirm():
            ...
            if not pid: return            
            #加入 controller_id 讓 Broker 知道是誰申請的
            payload = json.dumps({
                "bed_name": bed_name, 
                "sensor_id": sensor, 
                "patient_id": pid,
                "controller_id": self.my_controller_id  # <--- 在這！
            })
            print(f"[{self.my_controller_id}] 發送綁定請求: {payload}")
            #頻道是 req_bind，對齊 Broker 的設定
