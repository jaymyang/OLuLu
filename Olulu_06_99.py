...
def start_server(): #    持續接受新客戶端連線
    ...
    # target=scan_clients: 指定執行緒要執行的函式是 scan_clients。daemon=True: 將執行緒設定為「守護執行緒 (daemon thread)」。守護執行緒的特性是，當主程式結束時，它們會自動被終止。
    while True:
        if closing==True:
            break
        ...
        try:
            ...
            # 確保 `client_dict` 內的 IP 是最新的
            #with client_dict_lock:
            #    for j in client_dict:
            #        if client_dict[j][2] == client_address[0]:  
            #            client_dict[j][1] = "blue"  
            #            break 
...
def message_R(message_list, client_address): 
    global client_dict, connected_clients  

    client_name = message_list[-1]  # 取得感測器名稱，例如 'LuLu01'
    client_ip = client_address[0]  # 取得感測器的 IP 地址
    print(f"{client_name} 已連線，IP: {client_ip}")

    if client_name in ['LuLu01','LuLu02','LuLu03','LuLu05','LuLu06','LuLu07','LuLu08','LuLu09','LuLu17','LuLu18']:  # 檢查是否為合法感測器
     
        # 更新 client_dict 中的 IP
        for j in client_dict:
        ...
        dataDisplay_text.config(text=f"已連線的感測器：{connected_clients}")
...
def message_A(message_list):
    ...
    for i, entry in enumerate(data):
        if entry['name'] == new_name: #data字典中的name就是例如LuLu01等的ID
            ...
            found = True
            break
    if not found and new_name in ['LuLu01','LuLu02','LuLu03','LuLu05','LuLu06','LuLu07','LuLu08','LuLu09','LuLu17','LuLu18']: #資料格式如下，以感測ID為準存檔；但如尚無ID則應跳過
        with data_lock:
...
def scan_clients():
    ...
    # 每分鐘的25秒遍歷已登錄且連線中病人的time，如無符合目前時間的資料，就append.既有串列裡最後一個補足資料缺口
        if time.localtime(time.time()).tm_sec == 25 and len(data)>0 :#
            for j in range(len(data)): #檢查既有資料名單。這邊查詢的方式改變，是因為data中，只有已登錄ID的床號與病歷號，才會有資料，所以_info_data與data的數據順序不再同步
                ...
                if len(data[j]['weight']) >0: #如某床位已有重量資料
                    for k in range(len(pt_info_data)):
                        if pt_info_data[k]['client_IP'] !="離線" and pt_info_data[k]['client_name']==data[j]['name'] : #確認仍連線
...
def assign_bed(button_number,assign_bed_displayed): #button_number就是所選的按鈕編號，由0起算
    global button_on_display, data, pt_info_data, connected_clients,client_dict
...
#-----------------次函式：確認使用者的選擇----------------  
    def confirm_selection(button_number):    #按下確認後處理感測器選擇與病歷號輸入內容
        ...
        if not selected_value:  # 如果沒有選擇
            messagebox.showwarning("注意，請選擇一個感測器！")
            return
        ...
        if patient_id !='':#假如並非空白
            ...
            #    if client_dict[i][1] == chosen_client: #選擇了感測器
                #if client_dict[i][1] == pt_info_data[button_on_display]["client_name"]: #選擇了感測器
            #        print(client_dict[i],chosen_client)
                  
            #        with pt_info_data_lock:# 更新該感測器的 IP
            #            pt_info_data[button_on_display]['client_IP'] = client_dict[i][2]
            #            print('pt_info_data[button_on_display]',pt_info_data[button_on_display]['client_IP'])
            #        break  # 找到後直接跳出，避免重複比對
            #print('pt_info_data[button_on_display]',pt_info_data[button_on_display]['client_IP'])
            with client_dict_lock:
            ...
            dataDisplay_text.config(text=f"已連線的感測器：{connected_clients}") #顯示已連線的感測器
...
style = ttk.Style()
style.theme_use('alt')
style.configure("TFrame", background="#FFFAF4") #框架背景色
style.configure("TButton", font=("Arial", 12), padding=5,background="#FBFBFF") #右側按鈕色
style.configure("Selected.TButton", background="orange", foreground="white")#右側按鈕選擇色
...
try:
    ...
    canvas.create_image(380, 300, image=init_image_tk, anchor="center")
except Exception as e:
    print(f"載入初始圖片時發生錯誤：{e}")
# 顯示使用者點選床位的資料
dataDisplay_text = tk.Button(left_frame, text="點選右方按鈕註冊病人", bg="white", font=("Arial", 12),state="disabled",relief="flat")
dataDisplay_text.grid(column=1,row=1)
    
#功能按鈕，注意尺寸大小單位是字元
switch_button = tk.Button(left_frame, text="切換1或8小時資料", width=20, height=1,command=toggle_switch, font=("Arial", 12),bg="#ECF5FF")
switch_button.grid(column=0,row=1)
logout_client_button = tk.Button(left_frame, text="登出病人", width=20, height=1,command=logout_client, font=("Arial", 12),bg="#ECF5FF")
